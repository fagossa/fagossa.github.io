...
# Cats-effect
The goal of this article is to document some _cats-effect_ techniques that could be helpful to start working with effects. The code is inspired on a talk made by *Paweł Szulc* at Scalapeño 2018 (originally based on _monix_).
...
As we are going to develop a console application we need a way to wrap thoses interactions with effects. The simplest behavior to read and write from the terminal can be expressed as:
...
// no need to indicate the implicitly directly on the caller site
...
* Using the MTL approach using an `ApplicativeAsk` which is basically a wrapper around a _Read Monad_. However, our performance would be (severily) penalized.
...
### Handling errors
To keep things simple I try to avoid _Monad Transformers_. In this particular case, we are going to use a `MonadError[F, E]` which is already provided by `Sync[F]`.
...
We are assuming our `E` is a subtype of `Throwable`. If you are going that way, at least be sure to not to carry the stacktrace with you:
